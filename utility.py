...
def get_statistics(labels, scores):
    ...
    return fpr_at_95_tpr, auroc, aupr, aupr_reverse
...
def load_standard_scaler(model_name, ind_name, layer_idx):
    filename = f"saved_models/{model_name}/{ind_name}_best/ss-{layer_idx}.joblib"
    if os.path.exists(filename):
        ss = load(filename)
    else:
        print(f"Standard Scaler of layer{layer_idx} not found!")
        return
    return ss


def load_standard_scaler_score(model_name, ind_name, layer_idx):
    filename = f"saved_models/{model_name}/{ind_name}_best/ss-score-{layer_idx}.joblib"
    if os.path.exists(filename):
        ss_score = load(filename)
    else:
        print(f"Standard Scaler for scores of layer{layer_idx} not found!")
        return
    return ss_score


def load_kde(model_name, ind_name, layer_idx):
    filename = f"saved_models/{model_name}/{ind_name}_best/kde-point2-{layer_idx}.joblib"
    if os.path.exists(filename):
        ss_score = load(filename)
    else:
        print(f"KDE of layer{layer_idx} not found!")
        return
    return ss_score


def calc_cumulative_params(model):
    n_params = []
    cumulative_params = 0
    for module in model.modules():
        ...
        if hasattr(module, 'weight') or hasattr(module, 'bias'):
            ...
            # If the module is a Conv2d layer, print the cumulative parameters
            if isinstance(module, nn.Conv2d):
                n_params.append(cumulative_params)
    n_params.append(cumulative_params)
    return n_params
