...
def save_inter_outputs(model_name, id_name, ds_name=None, is_training_set=False, model=None, overwrite=False):
        ...
        for data, _ in data_loader:
            ...
            for i in range(len(outputs)):
                if len(outputs[i].shape) == 4:
                    # inpt = np.transpose(outputs[i], (0, 3, 2, 1))
                    # size = stride = inpt.shape[1]/2
                    # layer = Avgpool_layer(input_shape=inpt.shape, size=size, stride=stride, pad=False)
                    ...
                    # outputs[i] = np.max(outputs[i], axis=(2, 3))  # batch_size x C
