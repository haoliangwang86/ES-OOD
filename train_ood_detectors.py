...
def find_best_ocsvm(model_name, ds_name, layer_idx, ood_error_rate):
    ...
    self_adaptive_shifting = SelfAdaptiveShifting(train_X)
    ...
    if ds_name == "cifar10":
        if args.model == "densenet100":
            gamma_candidates = [0.1, 0.25, 0.5, 1.0]
        else:
            gamma_candidates = [0.001, 0.0025, 0.005, 0.01, 0.025, 0.05, 0.1]
    else:  # cifar100
        if args.model == "densenet100":
            gamma_candidates = [0.25, 0.5]
        elif args.model == 'resnet34':
            gamma_candidates = [0.1, 0.25, 0.5]
        else:
            gamma_candidates = [0.001, 0.0025, 0.005, 0.01, 0.025, 0.05, 0.1]
    ...
    filename = basedir + f"ss-{layer_idx}.joblib"
    dump(ss, filename)
    ...
    # fit standard Gaussian using InD scores
    ind_train_scores = -best_model.decision_function(X)
    ind_train_scores = ind_train_scores.reshape(-1, 1)
    ss_score = StandardScaler()
    ss_score.fit(ind_train_scores)

    filename = basedir + f"ss-score-{layer_idx}.joblib"
    dump(ss_score, filename)
